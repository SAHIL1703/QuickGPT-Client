        ...
        {plans.map((plan) => (
          ...
          >
            <div className="flex-1 flex flex-col">
              ...
              <ul className="flex-1 list-disc list-inside text-sm text-gray-700 dark:text-purple-200 space-y-1">
                {plan.features.map((feature,index)=>(
                  <li key={index}>{feature}</li>
                ))}
              </ul>
              <button className="mt-6 bg-purple-600 hover:bg-purple-700 text-white active:bg-purple-800 font-medium py-2 rounded transition-colors cursor-pointer">Buy Now</button>
            </div>
          </div>
        ))}
      </div>
